 ...
 * 该结构体表示一个时间[1]P216. 
 * 
 * RTC时钟时钟存储器中的值都是以BCD码的形式保存的,
 * 但是Linux驱动程序中使用二进制码形式, 两者之间
 * 有专用的转换函数[1]P217.
 *
...
struct rtc_time {
	int tm_sec; 	/*!< 秒[1]P217. */
	int tm_min;  	/*!< 分[1]P217. */
	int tm_hour; 	/*!< 时[1]P217. */
	int tm_mday;    /*!< 天[1]P217. */
	int tm_mon; 	/*!< 月[1]P217. */
	int tm_year; 	/*!< 年[1]P217. */
	int tm_wday; 	/*!< 未用[1]P217 */
	int tm_yday; 	/*!< 未用[1]P217 */
	int tm_isdst; 	/*!< 未用[1]P217 */
};
