...
/**
 * 为了方便管理, Linux内核将LCD驱动程序归入平台设备的范畴[1]P287.
 *
 * @see Linux驱动开发入门与实战(2)
 */
struct platform_device s3c_device_lcd = {
	.name		  = "s3c2410-lcd", 	/* 平台设备的名字[1]P288. */
	.id		  = -1, 				/* 一般为-1[1]P288. */
	.num_resources	  = ARRAY_SIZE(s3c_lcd_resource), /* 资源数量[1]P288. */
	.resource	  = s3c_lcd_resource, /* 资源的指针[1]P288. */
	.dev              = {
...
static struct resource s3c_rtc_resource[] = {
	[0] = {
		.start = S3C24XX_PA_RTC, 		/* 看门狗IO内存开始位置, 被定义为WTCON的地址0x53000000[1]P210. */
		.end   = S3C24XX_PA_RTC + 0xff, /* 256字节的内存地址空间[1]P210. */
		.flags = IORESOURCE_MEM, 		/* IO内存资源[1]P210. */
	},
	[1] = {
		.start = IRQ_RTC, 				/* RTC的滴答中断号被定义为45[1]P210. */
		.end   = IRQ_RTC, 				/* RTC滴答的结束中断号[1]P210. */
		.flags = IORESOURCE_IRQ, 		/* 中断的IRQ资源[1]P210. */
	},
	[2] = {
		.start = IRQ_TICK, 				/* RTC的报警中断号, 被定义为24[1]P210. */
		.end   = IRQ_TICK, 				/* RTC的报警结束中断号[1]P210. */
		.flags = IORESOURCE_IRQ 		/* 中断的IRQ资源[1]P210. */
	}
};
...
struct platform_device s3c_device_rtc = {
	.name		  = "s3c2410-rtc", 		/* 平台设备的名字[1]P209. */
	.id		  = -1, 			   		/* 一般设为-1[1]P209. */ 
	.num_resources	  = ARRAY_SIZE(s3c_rtc_resource), /* 资源数量[1]P209 */
	.resource	  = s3c_rtc_resource, 	/* 资源的指针[1]P209. */
};
