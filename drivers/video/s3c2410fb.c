...
/**
 * 检查fb_var_screeninfo的值是否在合法的范围之内[1]P144.
 *
 *	s3c2410fb_check_var():
...
static int s3c2410fb_init_registers(struct fb_info *info)
{
	...
	local_irq_save(flags); /* 关闭中断, 在关闭中断前, 中断的当前状态被保存在flags中, 对于关闭中断的函数,
							* Linux内核有很多种, 可以查阅相关的资料[1]P144. */

	/* modify the gpio(s) with interrupts set (bjd) 
	 * 
	 * 下面的modify_gpio()函数是修改处理器GPIO的工作模式, 它的实现很简单, 将第2个参数的值与第3个参数
	 * 的反码按位与操作后, 写到第1个参数里. 这里的第1个参数实际就是硬件的GPIO控制器[1]P144.
	 */
				  ...
				  enum s3c_drv_type drv_type)
{
	...
	/* 获得在内核中定义的FrameBuffer平台设备的LCD配置信息结构体数据[2]P292. */
	...
	/* 申请s3c2410fb_info结构体空间以存储FrameBuffer设备相关的数据[2]P292. */
	...
	/* 将LCD的IO端口占用的这段IO空间映射到内存虚拟地址[2]P292. */
	 ...
	 * 分配DRAM内存给FrameBuffer, 并且初始化这段内存[2]P292. */
	...
	/* 初始化LCD控制器的相关寄存器[2]P292. */
	s3c2410fb_init_registers(fbinfo);

	/* 检查FrameBuffer的相关参数, 如果传递的参数不合法则进行修改[2]P292. */
	...
	/* create device files
	 * 对设备文件系统的支持, 创建framebuffer的设备文件[2]P292. */
...
{
	struct fb_info *fbinfo = platform_get_drvdata(pdev); /* 从平台设备中获得fb_info信息[1]P145. */
	struct s3c2410fb_info *info = fbinfo->par; /* 获得私有数据[1]P145. */
	int irq;

	unregister_framebuffer(fbinfo);  		/* 注销帧缓存设备[2]P293. */
	s3c2410fb_cpufreq_deregister(info);

	s3c2410fb_lcd_enable(info, 0); 			/* 停止LCD控制器[1]P145. */
	msleep(1); 								/* 等待LCD停止[1]P145. */

	s3c2410fb_unmap_video_memory(fbinfo); 	/* 释放显示缓冲区[2]P293. */

	/* 释放时钟源[2]P293. */
	if (info->clk) { 			/* 停止时钟[1]P145. */
		clk_disable(info->clk);
		clk_put(info->clk);
		info->clk = NULL;
	}
	...
	/* 释放IRQ中断[2]P293. */
	irq = platform_get_irq(pdev, 0); /* 得到中断线, 以便释放[1]P145. */
	free_irq(irq, info); 			 /* 释放中断线[1]P145. */

	iounmap(info->io); 	/* 取消寄存器内存映射[2]P293. */

	release_resource(info->mem); /* 释放内存区域占用的资源[2]P293. */
	kfree(info->mem); 	/* 释放内存[2]P293. */

	platform_set_drvdata(pdev, NULL);
	framebuffer_release(fbinfo); /* 向内核注销FrameBuffer设备[2]P294. */
...
static struct platform_driver s3c2410fb_driver = {
	.probe		= s3c2410fb_probe, /* 初始化函数, 在platform_driver_register中被调用[1]P139. */
	.remove		= __devexit_p(s3c2410fb_remove),
	.suspend	= s3c2410fb_suspend,
	.resume		= s3c2410fb_resume,
	.driver		= {
		.name	= "s3c2410-lcd", /* 设备名字, 一定要和platform_device中的name一样
								  * 才能把平台驱动和前面定义的平台数据联系起来[1]P139. */
		.owner	= THIS_MODULE,
	},
};
...
/**
 * 2410 LCD初始化函数[1]P138.
 *
 * LCD控制器驱动程序的加载函数[2]P286.
 *
 * @see Linux系统移植(2)
 * @see Linux驱动开发入门与实战(2)
 */
int __init s3c2410fb_init(void)
{
	...
	/* 
	 * 上面注册成功后返回0, 那么这里为什么还有注册一次呢?
	 * 
	 * 答: platform_driver_register()会调用s3c2410fb_probe(),
	 * 第1次调用platform_driver_register()成功, 并不代表探测
	 * 函数s3c2410fb_probe()成功. 因此为了提高成功率, 有注册
	 * 了一次[2]P287.
	 */
	if (ret == 0)
...
/**
 * LCD控制器驱动程序卸载函数[1]P287.
 *
 * @see Linux驱动开发入门与实战(2)
 */
static void __exit s3c2410fb_cleanup(void)
{
	/* 由于在s3c2410bf_init()中注册了两次, 所以这里对应的要卸载两次[1]P287. */
