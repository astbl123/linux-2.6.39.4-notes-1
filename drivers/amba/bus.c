...
{
	struct amba_device *pcdev = to_amba_device(dev); /* 由devices转换为自定义的设备结构[1]P193. */
	int retval = 0;

	retval = add_uevent_var(env, "AMBA_ID=%08x", pcdev->periphid); /* 向env添加一个新的变量AMBA_ID[1]P193. */
	return retval;
}
#else
#define amba_uevent NULL 			/* 不支持热插拔事件[1]P193. */
#endif
